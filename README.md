...
如果你已經安裝 DashPi，但當時沒有使用 `-W epd7in3e`，通常不需要重灌。可以手動下載 driver、修改設定檔，然後重開機。
...
下載 Waveshare 7.3 吋 E 的 driver：

```bash
curl -fsSL -o src/display/waveshare_epd/epd7in3e.py \
  https://raw.githubusercontent.com/waveshareteam/e-Paper/master/RaspberryPi_JetsonNano/python/lib/waveshare_epd/epd7in3e.py
